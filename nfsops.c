...
static __always_inline int path_is_nfs_client(struct path *p)
{
	struct dentry *de;
	...
	if (!p)
		return 0;
	if (bpf_probe_read_kernel(&de, sizeof(de), &p->dentry))
		return 0;
	if (!de)
		return 0;
	if (bpf_probe_read_kernel(&sb, sizeof(sb), &de->d_sb))
		return 0;
	if (!sb)
		return 0;
	if (bpf_probe_read_kernel(&magic, sizeof(magic), &sb->s_magic))
		return 0;
